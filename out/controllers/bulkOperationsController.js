...
class BulkOperationsController {
    ...
    async showBulkStatusCheck() {
        try {
            ...
            const action = await vscode.window.showInformationMessage(`📊 Status Summary: ${cleanCount} clean, ${dirtyCount} with changes`, 'View Details', 'Switch to Worktree', 'Show in Output');
    ...
    async createWorktreesForAllBranches() {
        try {
            ...
            const confirmation = await vscode.window.showInformationMessage(`🌳 Create worktrees for ${branchesWithoutWorktrees.length} branch(es)?\n\n${previewMessage}`, { modal: true }, 'Create Worktrees', 'Cancel');
                ...
                try {
                    ...
                    if (!token.isCancellationRequested) {
                        ...
                        if (result.errors.length > 0) {
                            ...
                            vscode.window.showWarningMessage(`${summaryMessage}\n\nErrors:\n${errorDetails}${moreErrors}`, 'View Logs').then(selection => {
